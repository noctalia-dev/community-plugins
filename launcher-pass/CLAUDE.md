...
5. **Cleanup** — ~~delete `Main.qml`, `LauncherProvider.qml`, `Settings.qml`, `manifest.json`, `settings.json`, `i18n/`~~ **DONE.** `preview.png` deliberately kept until step 4 consumes it (it's the thumbnail source); delete it in the same commit as `thumbnail.webp`. `launcher.luau`'s header comment was also refreshed (dropped the "Port step 1/2" framing and the `LauncherProvider.qml` reference).
