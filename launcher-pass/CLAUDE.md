...
| `launcher.luau` | Browse mode done. Detail mode (`pass show`, parsing, row list, "Go back", pinentry dance) done. **All Copy + Type actions wired.** Copy: Password/OTP via `pass -c` / `pass otp -c` (copyViaPass), Username/fields via `noctalia.copyToClipboard` (copyPlain). Type: `wtype -d <wtypeDelay> -- <value>` after a launcher close + `sleep <typeDelay/1000>` (typeValue); OTP typed via `pass otp` → wtype (typeOtp). Type rows are hidden entirely when `wtype` is absent (`wtypeAvailable`). |
