...
- **Fuzzy *ranking* uses `noctalia.fuzzyScore`** (match semantics unchanged — see "Behaviour to preserve #4"). `rankEntry` sums `noctalia.fuzzyScore(fragment, path)` over the query fragments — the host's native fzy scorer (case-insensitive; exact = 1024; prefix / word-boundary / consecutive-run bonuses; longer text penalised). Summing *per fragment* (not passing the whole query as one fzy pattern) keeps the match order-independent and the inter-fragment space a wildcard. A fragment the scorer rejects scores 0 — the row already passed `matchesFragments`, so it stays, just ranked low. Ties break on fewer path segments, then alphabetically. This is step 6: the scoring moves from a Luau double loop into the C scorer, which matters over the capped grep hit set on a large store. (Earlier iterations: the QML base-27 segment score let leaf *length* dominate; the interim `rankEntry` counted whole-segment-equal fragments then depth.)
...
6. **Optimizations** — ~~use native `noctalia.fuzzyScore()` for ranking~~ **DONE.** `rankEntry` now returns `Σ noctalia.fuzzyScore(fragment, nameLower)` (+ depth for the tiebreak) instead of a Luau segment-equality loop; the sort key in `renderRows` follows. Matching is untouched — `matchesFragments` + the `grep -iF` pre-filter still decide *which* rows appear; fuzzyScore is purely the sort key. Per-fragment summing (rather than one whole-query fzy pattern) preserves order-independence and the space-as-wildcard rule. No `plugin_api` bump (`fuzzyScore` is base-lib). Behaviour to preserve #4 note updated.
