...
**Browse mode, detail-mode listing, and every row action (Copy + Type) are done. The v4 files are deleted.** Only `preview.png` remains from the v4 set, kept as the source for `thumbnail.webp` (step 4); it goes once the thumbnail exists.
...
| `translations/*.json` | Complete. 11 locales (`en de fr it es ja nl pt ru tr zh-CN`), 39 keys each, identical key set (verified by script). Legacy `i18n/` deleted. |
...
- **Settings are top-level `[[setting]]` blocks, not `[[launcher_provider.setting]]`.** The manifest docs only document nested settings for `widget`/`panel`; every sibling launcher plugin (`pass`, `file-search`, `k8s-status`) uses top-level `[[setting]]`, and for a launcher-only plugin the two are equivalent. Seven settings: `storePath` folder, `clipTimeout` string, `typeDelay`/`wtypeDelay`/`pinentryGraceMs` int (`advanced`), and `detailActionOrder`/`detailActionGrouping` select (`advanced`) — the latter two shape the detail-view Copy/Type layout (see step 7). `select` options use inline-table arrays `options = [{ value, label_key }, …]` (the idiom across sibling plugins), each `label_key` a `settings.<key>.options.<value>` translation key.
...
- **Store index is a *file* that `grep` filters per keystroke — no Luau work does the filtering.** `buildIndex` runs one `find … -name '.*' -prune -o -type f -name '*.gpg' -printf '%P\n' -o -type d -printf '%P/\n' > <pluginDataDir>/store.index` (shell string so it can redirect); the callback just sets `indexReady` / `indexAt` — near-zero Luau, safe at module load. Every `onQuery` then greps that file: search → `searchGrepCmd` (`grep -iF -e <f1> file | grep -iF -e <f2> | … | head -n SCAN_CAP=500`), browse → `browseGrepCmd` (`grep -E '^<reEsc folder>/[^/]+/?$' file | head -n BROWSE_CAP=2000`; root pattern `^[^/]+/?$`). The grep runs in a subprocess **off the 25 ms callback budget**; Luau only ever `parseHits` the capped output. Rebuilt in the background past `INDEX_TTL_MS` (60 s), grepping the current file meanwhile. This replaced two failed attempts: (a) the original `find <store> … | grep | head` **re-walked the whole tree per distinct query**; (b) an interim all-in-Luau index (`parseIndex` into a `{…}` array, filtered in `onQuery`) — parsing ~1.5 k+ lines into tables at load, and `matchesFragments` over the whole array per keystroke, both blew the budget on a large store (`parseIndex` at `:227`).
- **Ranking: cheap Luau `prerank` over the grep hits, then a bounded `noctalia.fuzzyScore` pass** (match semantics unchanged — see "Behaviour to preserve #4"). `prerank(relLower, frags)` (pure Luau, no C crossings) → `(segHits, depth)`: fragments equal to a whole `/`-bounded segment, then segment count. The top `SCORE_POOL` (64, just above `MAX_RESULTS`) by that key then get `fuzzySum` = `Σ noctalia.fuzzyScore(fragment, relLower)` — the host's native fzy scorer (case-insensitive; exact = 1024; prefix / word-boundary / consecutive-run bonuses). Only the *visible page* is fully fzy-ordered. **`fuzzyScore` is called ≤ SCORE_POOL times per keystroke** — each call crosses into C with a ~26 KB stack frame, and "fuzzyScore every hit" (step 6 as first shipped) is what pushed the callback past 25 ms in the first place. Per-fragment sum (not one whole-query fzy pattern) keeps the match order-independent and the inter-fragment space a wildcard; a fragment fzy rejects scores 0. Ties: fewer segments, then `rel` alphabetical. `renderSearchRows` re-checks `matchesFragments` against each hit's path *relative to* `folder` (grep matched the raw line); `renderBrowseRows` just sorts the already-direct-children hits by basename. Rows show the folder-relative path (search) or basename (browse). (Earlier iterations: QML base-27 segment score; an interim `rankEntry` = `Σ fuzzyScore` over *every* match.)
...
- **Listing** — `buildIndex` writes `<pluginDataDir>/store.index` (one `find`, all `*.gpg` + all folders, `%P` relative, hidden `-prune`d). Every `onQuery` greps that file in a subprocess: `browseGrepCmd` (`grep -E` for exact direct children, `| head -n 2000`) or `searchGrepCmd` (`grep -iF` AND-chain, `| head -n 500`). Luau `parseHits` only the capped output. `renderSearchRows` re-checks `matchesFragments` on the path relative to `folder`, then prerank → `fuzzySum` on the top `SCORE_POOL`; `renderBrowseRows` sorts by basename. Folders appear as search hits only when their own path matches. No tree re-walk per query; no filtering on the Luau CPU budget.
- **Caching** — the index file is rebuilt in the background past `INDEX_TTL_MS` (60 s), grepping the current file meanwhile; decrypted detail entries cached 60 s per entry path (so a debounce re-fire while a detail view is open doesn't re-run `pass show` / pinentry). The old per-query `listCache` and browse-prefetch tree are gone.
...
2. **Translations** — ~~add the new keys in `en.json`; port `de fr it es ja nl pt ru tr zh-CN` from `i18n/`~~ **DONE.** All 11 `translations/*.json` carry the same key set as `en.json` (31 keys then; 39 after step 7's two select settings) — nested JSON, `noctalia.tr` resolves dotted keys against the nesting. Legacy strings were reused from `i18n/`; new/reworded keys translated fresh: `detail.decrypting`, `detail.error`, `notification.copyFailed`, `notification.typeFailed`, `action.copyUsername`, `action.typeUsername`, `settings.pinentryGraceMs.{label,desc}`, and the reworded `settings.typeDelay.label` ("Type delay", not the old misleading "Launcher Close Delay") + `settings.clipTimeout.desc` (dropped the `{value}` placeholder). Dropped legacy-only keys not in the v5 code: `result.passwordEntry`, `settings.tab.{general,advanced}`. `nl`/`pt` legacy typos ("Gekopiëerd", "predefenido") fixed in passing. `i18n/` still on disk — removed in Cleanup (step 5). Generator + parity check: `$CLAUDE_JOB_DIR/tmp/gen_translations.py` (not committed).
3. **README** — ~~rewrite for v5~~ **DONE.** Rewritten to the community-plugins `README_TEMPLATE.md` layout (Plugin table / Requirements / Usage / Settings / IPC / Notes), matching the sibling `pass/README.md` style. Covers: `/pass` prefix (with the `provider_prefix` caveat), the required/optional dependency split, the detail-view rows + row filter, Copy-vs-Type and clipboard-auto-clear behaviour, the pinentry hide/reopen, the `noctalia msg panel-*` IPC (no custom plugin IPC), and the auto-paste limitation. No `## Features` / `## License` sections — the template and siblings omit them.
4. **`thumbnail.webp`** — ~~convert/regenerate from `preview.png`.~~ **DONE.**
5. **Cleanup** — ~~delete `Main.qml`, `LauncherProvider.qml`, `Settings.qml`, `manifest.json`, `settings.json`, `i18n/`, `preview.png`~~ **DONE.** `preview.png` was removed alongside the new `thumbnail.webp` (step 4). `launcher.luau`'s header comment was also refreshed (dropped the "Port step 1/2" framing and the `LauncherProvider.qml` reference).
6. **Optimizations** — ~~use `noctalia.fuzzyScore()` for ranking~~; **then, after a bug report, the whole listing/ranking path was reworked. DONE.**

   *First cut:* `rankEntry` = `Σ noctalia.fuzzyScore(fragment, path)` over the matched set, replacing a Luau segment loop. **This regressed:** on a large store a broad query (`w`) matched ~500 grep hits + ~400 synthesised parent rows, and `fuzzyScore` over all ~900 (each call a Lua→C crossing with a ~26 KB fzy stack frame) blew the host's 25 ms async-callback budget — the callback aborted before `setResults`, so the launcher sat on "Loading…" forever and Noctalia disabled the plugin. Error: `script callback 'async command callback' exceeded its CPU budget`.

   *Fix (this step):*
   - **Index file + grep** — `buildIndex` writes `<pluginDataDir>/store.index` via `find … > file` (shell string); the callback only sets `indexReady`/`indexAt`, so it is trivial even at module load (the interim all-in-Luau `parseIndex` blew the load budget at `:227` on a large store). `onQuery` greps that file (`searchGrepCmd` / `browseGrepCmd`, both `| head`-capped) in a subprocess — filtering is C, off the 25 ms budget. Background rebuild past `INDEX_TTL_MS`, serving the current file meanwhile. Removed: `listCache`, `parseEntries`, `searchCmd`/`browseArgv`/`browseCacheKey`, `folderDir`, `prefetchBrowse`/`prefetchChildren`/`pendingFetch`, `GREP_CAP`/`BROWSE_TTL_MS`/`SEARCH_TTL_MS`. New: `shq` (back), `reEsc`, `indexPath`, `parseHits`, `renderBrowseRows`/`renderSearchRows` (was `renderRows`).
   - **Bounded fuzzyScore** — `renderSearchRows` does a cheap pure-Luau `prerank` (`segHits, depth`) over every grep hit, then `fuzzySum` (`Σ fuzzyScore`) over only the top `SCORE_POOL = 64` (just over `MAX_RESULTS`). fuzzyScore ≤ 64×/keystroke, not ~900×.
   - Match semantics unchanged (`matchesFragments`, whitespace-split, contiguous substrings, order-independent, space = wildcard). Search rows show the folder-relative path; browse rows the basename.
   - Measured with the real `luau` against `~/.password-store` (843 entries / 626 folders): every keystroke ~1.5 ms of Luau (was blowing the 25 ms budget), flat over 1000 repeats; the grep is a subprocess and doesn't count against the budget.
   - No `plugin_api` bump.
7. **Customization** — ~~add settings for detail-row order & grouping~~ **DONE.** Two `select` settings (`advanced`): `detailActionOrder` (`copy` | `type` — which verb's row comes first per value; default `copy`) and `detailActionGrouping` (`interleaved` | `grouped` — pair each value's Copy+Type rows, or emit every Copy row then every Type row; default `interleaved`). `renderDetailRows` now builds a `specs` list in the fixed value order (password, OTP, username, fields), resolves a `verbs` list from `detailActionOrder` (with `type` dropped when `wtype` is absent), then nests spec×verb one way or the other. `addAction` became `actionRow(entryPath, spec, verb, n)` returning one row + recording its `detailActions[act:<n>]` context; the id counter `n` is now explicit (was `#rows`). Default (`copy`/`interleaved`) reproduces the previous layout byte-for-byte. 8 new translation keys × 11 locales added via the regenerated generator (now also emits `en.json`, byte-identical to the hand-maintained file).
...
| custom `fuzzyMatch` | *matching* reimplemented in Luau (`matchesFragments`) to keep exact semantics — `noctalia.fuzzyScore` is subsequence/order-dependent so it can't drive matching; *ranking* does use `noctalia.fuzzyScore(pattern, text)`, summed per fragment (see `rankEntry`) |
...
4. **Fuzzy match semantics** (`fuzzyMatch` in the QML): lowercase; split query on whitespace; **every part must appear as a contiguous substring** of the target (spaces act as wildcards between fragments); non-match → excluded. Cap at 50 rows. Reimplement with plain (non-pattern) `string.find(hay, needle, 1, true)`. **The *matching* rule is preserved as-is.** The *ranking* uses `noctalia.fuzzyScore` (see Port status → `rankEntry`): `Σ fuzzyScore(fragment, path)` over the fragments, then fewer segments, then alphabetical. (The QML base-27 "earlier path segments" score made leaf length dominate; an interim Luau version counted whole-segment-equal fragments.)
