...
Install `tailscale` (and a running `tailscaled`) on the machine. Manifest also expects `jq` for a slim prefs extract.
...
| `admin_url` | `string` | _(empty)_ | Override admin console URL. |
...
- Shells out to `tailscale status --json`, `tailscale set …`, `exit-node list`, and `jq` for prefs.
