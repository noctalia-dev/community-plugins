import os
import socket
import struct
import sys

MAC = "78:99:87:AD:44:BA"
CH = 29

PATTERN = 0x9999
BLOCK = 16
SEQ = bytes([0x11, 0x22, 0x33, 0x33, 0x22, 0x11, 0x11, 0x22,
             0x33, 0x33, 0x22, 0x11, 0x11, 0x22, 0x33, 0x33])
COEFFICIENTS = [
    [2, 1, 1, 1, 4, 2, 1, 1, 2, 2, 4, 2, 4, 4, 16, 8],
    [2, 1, 1, 1, 4, 2, 1, 1, 1, 1, 2, 1, 2, 2, 8, 4],
    [1, 1, 4, 2, 2, 2, 4, 2, 16, 8, 4, 4, 2, 1, 1, 1],
    [1, 1, 4, 2, 1, 1, 2, 1, 8, 4, 2, 2, 2, 1, 1, 1],
    [16, 8, 2, 2, 4, 2, 4, 4, 1, 1, 4, 2, 1, 1, 2, 1],
    [8, 4, 1, 1, 2, 1, 2, 2, 1, 1, 4, 2, 1, 1, 2, 1],
    [2, 2, 4, 2, 4, 4, 16, 8, 2, 1, 1, 1, 4, 2, 1, 1],
    [1, 1, 2, 1, 2, 2, 8, 4, 2, 1, 1, 1, 4, 2, 1, 1],
    [4, 2, 4, 4, 16, 8, 2, 2, 1, 1, 2, 1, 1, 1, 4, 2],
    [2, 1, 2, 2, 8, 4, 1, 1, 1, 1, 2, 1, 1, 1, 4, 2],
    [4, 4, 16, 8, 1, 1, 2, 1, 4, 2, 1, 1, 4, 2, 2, 2],
    [2, 2, 8, 4, 1, 1, 2, 1, 4, 2, 1, 1, 2, 1, 1, 1],
    [1, 1, 2, 1, 1, 1, 4, 2, 4, 4, 16, 8, 2, 2, 4, 2],
    [1, 1, 2, 1, 1, 1, 4, 2, 2, 2, 8, 4, 1, 1, 2, 1],
    [4, 2, 1, 1, 2, 1, 1, 1, 4, 2, 2, 2, 16, 8, 4, 4],
    [4, 2, 1, 1, 2, 1, 1, 1, 2, 1, 1, 1, 8, 4, 2, 2],
]
EXP_TAB = bytes([
    ,
])
LOG_TAB = bytes([
    ,
])
BIAS_MATRIX = [
    bytes([0x46, 0x97, 0xB1, 0xBA, 0xA3, 0xB7, 0x10, 0x0A, 0xC5, 0x37, 0xB3, 0xC9, 0x5A, 0x28, 0xAC, 0x64]),
    bytes([0xEC, 0xAB, 0xAA, 0xC6, 0x67, 0x95, 0x58, 0x0D, 0xF8, 0x9A, 0xF6, 0x6E, 0x66, 0xDC, 0x05, 0x3D]),
    bytes([0x8A, 0xC3, 0xD8, 0x89, 0x6A, 0xE9, 0x36, 0x49, 0x43, 0xBF, 0xEB, 0xD4, 0x96, 0x9B, 0x68, 0xA0]),
    bytes([0x5D, 0x57, 0x92, 0x1F, 0xD5, 0x71, 0x5C, 0xBB, 0x22, 0xC1, 0xBE, 0x7B, 0xBC, 0x99, 0x63, 0x94]),
    bytes([0x2A, 0x61, 0xB8, 0x34, 0x32, 0x19, 0xFD, 0xFB, 0x17, 0x40, 0xE6, 0x51, 0x1D, 0x41, 0x44, 0x8F]),
    bytes([0xDD, 0x04, 0x80, 0xDE, 0xE7, 0x31, 0xD6, 0x7F, 0x01, 0xA2, 0xF7, 0x39, 0xDA, 0x6F, 0x23, 0xCA]),
    bytes([0x3A, 0xD0, 0x1C, 0xD1, 0x30, 0x3E, 0x12, 0xA1, 0xCD, 0x0F, 0xE0, 0xA8, 0xAF, 0x82, 0x59, 0x2C]),
    bytes([0x7D, 0xAD, 0xB2, 0xEF, 0xC2, 0x87, 0xCE, 0x75, 0x06, 0x13, 0x02, 0x90, 0x4F, 0x2E, 0x72, 0x33]),
    bytes([0xC0, 0x8D, 0xCF, 0xA9, 0x81, 0xE2, 0xC4, 0x27, 0x2F, 0x6C, 0x7A, 0x9F, 0x52, 0xE1, 0x15, 0x38]),
    bytes([0xFC, 0x20, 0x42, 0xC7, 0x08, 0xE4, 0x09, 0x55, 0x5E, 0x8C, 0x14, 0x76, 0x60, 0xFF, 0xDF, 0xD7]),
    bytes([0xFA, 0x0B, 0x21, 0x00, 0x1A, 0xF9, 0xA6, 0xB9, 0xE8, 0x9E, 0x62, 0x4C, 0xD9, 0x91, 0x50, 0xD2]),
    bytes([0x18, 0xB4, 0x07, 0x84, 0xEA, 0x5B, 0xA4, 0xC8, 0x0E, 0xCB, 0x48, 0x69, 0x4B, 0x4E, 0x9C, 0x35]),
    bytes([0x45, 0x4D, 0x54, 0xE5, 0x25, 0x3C, 0x0C, 0x4A, 0x8B, 0x3F, 0xCC, 0xA7, 0xDB, 0x6B, 0xAE, 0xF4]),
    bytes([0x2D, 0xF3, 0x7C, 0x6D, 0x9D, 0xB5, 0x26, 0x74, 0xF2, 0x93, 0x53, 0xB0, 0xF0, 0x11, 0xED, 0x83]),
    bytes([0xB6, 0x03, 0x16, 0x73, 0x3B, 0x1E, 0x8E, 0x70, 0xBD, 0x86, 0x1B, 0x47, 0x7E, 0x24, 0x56, 0xF1]),
    bytes([0x88, 0x46, 0x97, 0xB1, 0xBA, 0xA3, 0xB7, 0x10, 0x0A, 0xC5, 0x37, 0xB3, 0xC9, 0x5A, 0x28, 0xAC]),
]

HEADER = bytes([0xFE, 0xDC, 0xBA])
TRAILER = 0xEF


def key_schedule(key_init):
    k = bytearray(key_init)
    k[15] ^= 6
    keys = [bytes(k)]
    register = list(k)
    register.append(sum(register) & 0xFF if False else 0)
    register[-1] = 0
    x = 0
    for b in k:
        x ^= b
    register[-1] = x & 0xFF
    for rnd in range(1, 17):
        for idx in range(17):
            v = register[idx]
            register[idx] = ((v >> 5) | ((v << 3) & 0xFF)) & 0xFF
        key_i = bytearray(16)
        for i in range(16):
            key_i[i] = (register[(rnd + i) % 17] + BIAS_MATRIX[rnd - 1][i]) & 0xFF
        keys.append(bytes(key_i))
    return keys


def encrypt(plaintext, keys):
    ct = bytearray(plaintext)
    pt = bytes(plaintext)
    for rnd in range(8):
        if rnd == 2:
            for i in range(BLOCK):
                if (PATTERN >> i) & 1:
                    ct[i] ^= pt[i]
                else:
                    ct[i] = (ct[i] + pt[i]) & 0xFF
        for i in range(BLOCK):
            if (PATTERN >> i) & 1:
                ct[i] ^= keys[rnd * 2][i]
            else:
                ct[i] = (ct[i] + keys[rnd * 2][i]) & 0xFF
        for i in range(BLOCK):
            b = ct[i]
            ct[i] = EXP_TAB[b] if (PATTERN >> i) & 1 else LOG_TAB[b]
        for i in range(BLOCK):
            if (PATTERN >> i) & 1:
                ct[i] = (keys[rnd * 2 + 1][i] + ct[i]) & 0xFF
            else:
                ct[i] ^= keys[rnd * 2 + 1][i]
        copy = bytes(ct)
        for i in range(BLOCK):
            s = 0
            for j in range(BLOCK):
                s = (s + COEFFICIENTS[i][j] * copy[j]) & 0xFF
            ct[i] = s
    for i in range(BLOCK):
        if (PATTERN >> i) & 1:
            ct[i] ^= keys[16][i]
        else:
            ct[i] = (ct[i] + keys[16][i]) & 0xFF
    return bytes(ct)


def challenge_response(challenge):
    return encrypt(SEQ, key_schedule(bytes(challenge)))


def encode(mtype, opcode, seq, payload):
    is_req = (mtype & 0x40) != 0
    plen = len(payload) + (1 if is_req else 2)
    out = bytearray(HEADER)
    out += bytes([mtype, opcode, (plen >> 8) & 0xFF, plen & 0xFF])
    if not is_req:
        out.append(0x00)
    out.append(seq & 0xFF)
    out += bytes(payload)
    out.append(TRAILER)
    return bytes(out)


def extract_messages(buf):
    msgs = []
    i = 0
    while True:
        j = buf.find(HEADER, i)
        if j < 0 or j + 8 > len(buf):
            break
        plen = (buf[j + 5] << 8) | buf[j + 6]
        total = plen + 8
        if j + total > len(buf):
            break
        if buf[j + total - 1] != TRAILER:
            i = j + 1
            continue
        raw = buf[j:j + total]
        mtype, opcode = raw[3], raw[4]
        is_req = (mtype & 0x40) != 0
        si = 7 if is_req else 8
        msgs.append({"type": mtype, "opcode": opcode, "seq": raw[si],
                     "payload": raw[si + 1:-1]})
        i = j + total
    return msgs, buf[i:]


def parse_battery(payload):
    i = 0
    res = {}
    while i + 1 < len(payload):
        ln = payload[i]
        idx = payload[i + 1]
        if idx == 0x07 and i + 5 <= len(payload):
            def lev(b):
                return -1 if b == 0xFF else (b & 0x7F)
            # last TLV wins (same as BudsLink loop)
            res = {"left": lev(payload[i + 2]), "right": lev(payload[i + 3]),
                   "case": lev(payload[i + 4])}
        i += ln + 1
    return res


